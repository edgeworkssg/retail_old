...
$(function () {
    ...
    DAL.fnGetInventoryLocationList(function (locations) {
        ...
        DAL.fnGetInventoryLocationsFromWS(function (locationsTo) {
            ko.applyBindings(new StockTransferViewModel(locations, locationsTo.records), document.getElementById("dynamicContent"));
        });
    });
    ...
    function StockTransferViewModel(locations, locationsTo) {
        var self = this;

        self.statusDropdown = ["ALL", "Pending", "Submitted", "Cancelled", "Received", "Rejected"];
        ...
        self.inventoryLocationToList = ko.observableArray(locationsTo); 
        ...
        self.inventoryLocationTo = ko.observable(ko.utils.arrayFirst(locationsTo, function (item) {
        ...
        // DAL.fnGetSetting("GoodsOrdering_ShowPriceLevelForStockTransfer", "yes", function (res) {
        //     if (res == "True") {
        ...
        self.openUpdatePage = function (hdr) {
            DAL.fnFetchStockTransferHdr(hdr.StockTransferHdrRefNo, function (result) {
                if (result.status == "") {
                    ...
                    self.isReadOnly(transferHdr.Status() == "Pending" ? false : true);
        ...
        self.submitPurchaseOrder = function () {
            ...
            BootstrapDialog.confirm("Are you sure you want to submit this Stock Transfer?", function (ok) {
                if (ok) {
                    self.currentStockTransfer().Status("Submitted");
                    self.SaveStockTransfer(function (success) {
                        if (success) {
                            self.isReadOnly(true);
                            self.backToList();
        ...
        self.isApprovedOrRejected = ko.computed(function () {
            if (self.currentStockTransfer()) {
                return self.currentStockTransfer().Status() == 'Approved' || self.currentStockTransfer().Status() == 'Rejected' || self.currentStockTransfer().Status() == 'Posted';
            }
            else {
                return false;
            };
        ...
        self.inventoryLocationFilteredDetailTo = ko.computed(function () {
            if (self.currentStockTransfer()) {
                var toLocation =  ko.utils.arrayFilter(self.inventoryLocationToList(), function (item) {
                                        return item.InventoryLocationName.toUpperCase() != "ALL" &&
        ...
        self.inventoryLocationFilterTextDetailKeyUpTo = function (obj, e) {
            ...
            var match = ko.utils.arrayFirst(self.inventoryLocationToList(), function (item) {
