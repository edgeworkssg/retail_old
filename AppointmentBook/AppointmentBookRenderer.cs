...
{
	public class AppointmentBookRenderer
	{
		...
		public AppointmentBookRenderer(AppointmentBookControlOptions options)
		{
			_options = options;
		}
		...
		public void RenderSchedule(Graphics g, List<TimeSlot> schedule, TimeSlot hoveredTimeSlot, TimeSlot selectedTimeSlot, TimeSlot resizedTimeSlot, bool drag, DateTime startTime, int x, int y, int w, int h)
		{
			startTime = TimeHelper.Floor(startTime, _options.TimeResolution);
			...
			while (cy < y + h)
			{
				var rect = new RectangleF(x, cy, w, _options.TimeIntervalHeight);
		...
		public void RenderSchedules(Graphics g, List<Employee> employees, TimeSlot hoveredTimeSlot, TimeSlot selectedTimeSlot, TimeSlot resizedTimeSlot, bool drag, DateTime startTime, int x, int y)
		{
			foreach (var employee in employees)
			{
				...
				RenderSchedule(g, employee.Schedule, hoveredTimeSlot, selectedTimeSlot, resizedTimeSlot, drag, startTime, rect.X, rect.Y, rect.Width, rect.Height);

				_pen.Color = Color.WhiteSmoke;
				_pen.Width = 1;
				g.DrawLine(_pen, rect.Right, rect.Top, rect.Right, rect.Bottom);

				x += _options.ScheduleColumnWidth;
			}
		}
