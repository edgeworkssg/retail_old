...
namespace PowerPOS.AccessLogSyncController
{
    public delegate void UpdateProgress(object sender, string message);
    public class SyncAccessLogRealTimeController
    {
            ...
            out string statusMsg, out DateTime LastModifiedOn)
        {
            try
            {
                ...
                AccessLogCollection myHdr = new AccessLogCollection();

                DataTable dt = SyncClientController.FetchAccessLogNotInServerWithPOSID(StartDate, numofRecords, PointOfSaleInfo.PointOfSaleID);
                ...
                if (dt != null)
                {
                    ...
                    {
                        myHdr[i].AccessLogID = new Guid(dt.Rows[i]["AccessLogID"].ToString());
        ...
        public bool SendRealTimeAccessLog()
        {
            try
            {
                #region Load Config
                ...
                if (!int.TryParse(AppSetting.GetSetting(AppSetting.SettingsName.Sync.SyncAccessLogRetrySecWhenDisconnected), out retrySecConnected))
                    retrySecConnected = 10;
                if (!int.TryParse(AppSetting.GetSetting(AppSetting.SettingsName.Sync.SyncAccessLogPerTime), out recordsPerTime))
                    recordsPerTime = 100;
                ...
                while (true)
                {
                    ...
                    if (OnProgressUpdates != null)
                        OnProgressUpdates(this, "Sync AccessLog Started. Connecting To Server");
                    tryTimes = 0;
                    ...
                    while (!isSuccessConnecting)
                    {
                        ...
                        try
                        {
                            lastSalesModifiedOn = ws.GetAccessLogLastTimeStamp(PointOfSaleInfo.PointOfSaleID);
                    ...
                    if (isSuccessConnecting)
                    {
                        ...
                        {
                            if (OnProgressUpdates != null)
                                OnProgressUpdates(this, "All data has been sent successfully to Server. Sync AccessLog is completed.");
                            Thread.Sleep(retrySecConnected * 1000);
                            continue;
                        }
                        ...
                        while (dataSent < dataCount)
                        {
                            ...
                            if (!GetLocalData(lastSalesModifiedOn, recordsPerTime > dataCount - dataSent ? dataCount - dataSent : recordsPerTime, 
                                out objAccessLog, out statusMsg, out lastModifiedOnPerTimes))
                            {
                                if (OnProgressUpdates != null)
                                    OnProgressUpdates(this, "Fetch Access Log Data Failed.");
                                Thread.Sleep(retrySecConnected * 1000);
                                continue;
                            }
                            if (OnProgressUpdates != null)
                                OnProgressUpdates(this, "Sending records #" + (dataSent + 1) + "-" + (dataSent + objAccessLog.Length) + " of " + dataCount + " to Server...");


                            try
                            {
                                result = ws.FetchAccessLogRealTime(objAccessLog);
                                if (result)
                                {
                                    lastSalesModifiedOn = lastModifiedOnPerTimes;

                                    if (OnProgressUpdates != null)
                                        OnProgressUpdates(this, "Sending records #" + (dataSent + 1) + "-" + (dataSent + objAccessLog.Length) + " of " + dataCount + " to Server is completed successfully.");
