...
WHERE object_id = OBJECT_ID(N'[dbo].[CustomerPricing]') AND type in (N'U'))
...
IF  NOT EXISTS (SELECT * FROM sys.objects 
WHERE object_id = OBJECT_ID(N'[dbo].[OutletCustomerPricing]') AND type in (N'U'))

BEGIN

CREATE TABLE [dbo].[OutletCustomerPricing](
	[OutletCustomerPricingID] [int] IDENTITY(1,1) NOT NULL,
	...
	[userint4] [int] NULL,
	[userint5] [int] NULL,
 CONSTRAINT [PK_[OutletCustomerPricing] PRIMARY KEY CLUSTERED 
(
	[OutletCustomerPricingID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
END