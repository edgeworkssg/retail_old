...
BEGIN
	...
	FROM item
	WHERE Attributes1 = @attributes1 and ISNULL(Deleted,0) = 0
	OPEN db_cursor   
	...
	SELECT Item.Attributes1 as ItemNo, Attributes2 as ItemName, Barcode, CategoryName, RetailPrice, FactoryPrice, MinimumPrice, 
			ItemDesc, IsServiceItem, IsInInventory, IsNonDiscountable, 
           IsCourse, CourseTypeID, Brand, ProductLine, Item.Attributes1, Attributes2, 
           ...
           ProductionDate, IsGST, hasWarranty, CreatedOn, CreatedBy, ModifiedOn, ModifiedBy, UniqueID,CASE WHEN SP.CountDeleted = 0 then 1 else 0 END as Deleted, 
	...
	LEFT JOIN
	(
		SELECT count(ItemNo) as CountDeleted, Attributes1
		FROM Item
		where ISNULL(Deleted,0) = 0
		GROUP BY Attributes1
	) SP ON SP.Attributes1 = Item.Attributes1
	WHERE (ItemNo = @itemno)
