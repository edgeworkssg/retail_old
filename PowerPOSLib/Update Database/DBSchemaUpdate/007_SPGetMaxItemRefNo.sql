...
BEGIN
	...
	from
	(
		select CAST(SUBSTRING(ItemNo, len(@pointofsaleid) + 3, len(itemno) - (len(@pointofsaleid) + 2)) as INT) as itemno from item where itemno like ''I0'' + cast(@PointOfSaleID as varchar) + ''%'' and ISNULL(userflag1,0) = 0
		union
		select CAST(SUBSTRING(attributes1, len(@pointofsaleid) + 3, len(attributes1) - (len(@pointofsaleid) + 2)) as INT) as itemno from item where itemno like ''I0'' + cast(@PointOfSaleID as varchar) + ''%'' and ISNULL(userflag1,0) = 1
	) as t
