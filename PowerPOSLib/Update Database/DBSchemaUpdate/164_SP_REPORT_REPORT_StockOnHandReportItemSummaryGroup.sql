IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[REPORT_StockOnHandReportItemSummaryGroup]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[REPORT_StockOnHandReportItemSummaryGroup]

SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[REPORT_StockOnHandReportItemSummaryGroup]') AND type in (N'P', N'PC')) BEGIN 
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[REPORT_StockOnHandReportItemSummaryGroup]
...
BEGIN
	...
	FROM	Item I
			...
			LEFT OUTER JOIN ItemSummaryGroup ISM WITH(NOLOCK) ON ISM.ItemNo = I.ItemNo and ISM.InventoryLocationGroupID = @LocationID
			LEFT JOIN (
				SELECT   ISM.ItemNo
						,ISM.SupplierID
						,SUPP.SupplierName
