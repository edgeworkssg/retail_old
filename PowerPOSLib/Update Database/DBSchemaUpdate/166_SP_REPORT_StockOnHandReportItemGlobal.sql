IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[REPORT_StockOnHandReportItemGlobal]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[REPORT_StockOnHandReportItemGlobal]

SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[REPORT_StockOnHandReportItemGlobal]') AND type in (N'P', N'PC')) BEGIN 
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[REPORT_StockOnHandReportItemGlobal]
...
BEGIN
	...
	SELECT DISTINCT 
			...
			,ISNULL(I.BalanceQuantity,0) AS Quantity 
			,ISNULL(I.BalanceQuantity,0) * I.RetailPrice  AS TotalRetail  
			,ISNULL(I.AvgCostPrice,I.RetailPrice)  AS COG 
			,ISNULL(I.BalanceQuantity,0) * ISNULL(I.AvgCostPrice,I.RetailPrice)  AS TotalCost 
			,I.Userfld1 AS UOM    
