...
FROM (
    SELECT ISNULL(oh.userfld5, oh.OrderRefNo) AS InvoiceNo, oh.OrderDate, it.ItemNo, it.ItemName, 
        oh.MembershipNo, mb.NameToAppear AS CustomerName, od.Quantity AS OrderQty, od.UnitPrice, od.Amount,
        ...
        ISNULL(od.userflag2, 0) AS IsNotified, (od.Amount - ISNULL(od.userfloat1, 0)) AS OutstandingBal,
        DeliveryStatus = CASE
                WHEN dod.OrderDetID IS NULL THEN ''Not Delivered''
                WHEN dod.IsDelivered = 1 THEN ''Delivered''
                ELSE ''Pending''
            END,
        oh.OrderHdrID, od.OrderDetID
    FROM OrderHdr oh
