...
namespace PowerWeb.API.Promo
{
    ...
    public class GetAvgPriceItemCategory : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            var CategoryName = context.Request.Params["CategoryName"];
            ...
            try
            {
                ...
                string query = "Select ISNULL(MAX(RetailPrice),0) As RetailPrice " +
                                "from Item where CategoryName = N'" + CategoryName + "'";
                ...
                string query3 = "select PromoCampaignName " +
                        ...
                        "where ISNULL(h.Deleted,0) = 0 and ISNULL(d.Deleted,0) = 0 and CONVERT(date, h.DateTo) > GETDATE() and d.CategoryName = '" + CategoryName + "' " +
                ...
                if (d.Rows.Count > 0)
                {
                    ...
                    result.Message = "This category are being used on other promos(" + existingpromo + "). Please check your item again and don't forget to set the priority right";
                }
                else
                {
                    result.Message = "";
