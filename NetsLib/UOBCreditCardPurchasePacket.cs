...
namespace PowerPOS.Nets
{
    ...
    /// </summary>
    class UOBCreditCardPurchasePacket : Packet
    {
        ...
        public UOBCreditCardPurchasePacket()
        {
            init();
        }
        ...
        public override byte[] toBytes()
        {
            ...
            #region Message Data 3 (Invoice No)
            /*idx = 0; //reset idx

            //Set cashback amount field code in message data
            temp = Utilities.asciiStrToBytesArray(NETSConstants.MessageData.INVOICE_NO.FieldCode);
            Array.Copy(temp, 0, msgData3, idx, temp.Length);
            idx += temp.Length;

            //set message data length
            temp = Utilities.intToBytesBCD(NETSConstants.MessageData.INVOICE_NO.Len, NETSConstants.MSG_LENGTH_BYTES_COUNT);
            Array.Copy(temp, 0, msgData3, idx, temp.Length);
            idx += temp.Length;

            myString = "00000000";
            if (!String.IsNullOrEmpty(this.uniqueNo))
                myString = ("00000000000000000000" + uniqueNo).Trim().Right(6);
            ...
            //set cashback amount in message data, set all to "0"
            for (int i = 0; i < NETSConstants.MessageData.INVOICE_NO.Len; i++)
            {
                toBytes = Encoding.ASCII.GetBytes(myArray[i]);
