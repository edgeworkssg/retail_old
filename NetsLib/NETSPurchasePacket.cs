...
namespace PowerPOS.Nets
{
    ...
    /// </summary>
    class NETSPurchasePacket : Packet
    {
        ...
        public void setRefNo(string refnos)
        {
            if (!refnos.Substring(2,1).Equals(" "))
                this.refno = refnos.Trim().Right(13);
            else
                this.refno = "             ";
        }
        ...
        {
            byte[] content = generateMessageHeader(NETSConstants.MessageHeader.FunctionCode.NETS_PURCHASE);
            ...
            byte[] msgData3 = new byte[NETSConstants.MessageData.ECR_REFERENCE_NUMBER.Len + 5]; //2 bytes field code, 2 bytes len, 1 byte separator
           ...
           // idx += temp.Length;
            ...
            idx = 0;
            //Set transaction amount field code in message data
            temp = Utilities.asciiStrToBytesArray(NETSConstants.MessageData.TRANSACTION_AMOUNT.FieldCode);
            ...
            string myString1 = this.refno.PadLeft(13,'0');
            ...
            idx = 0;
            temp = new byte[content.Length + msgDataTransactionType.Length + msgDataLoyaltyRedemption.Length + msgData.Length + msgData2.Length + msgData3.Length];
